    ...
    while (not interrupt and (keep_running or elapsed_time <= running)):
        try:
            ...
            while True:
                ...
                if use_top:
                    ...
                    if new_pid_stat[pid] and pid in old_pid_stat:
                        delta_time = float(new_pid_stat[pid][13]) + float(new_pid_stat[pid][14]) - float(
                            old_pid_stat[pid][13]) - float(old_pid_stat[pid][14])
                ...
                if write_file:
                    ...
                    proc_result += row_formatter.format(pid, str(cpu) + '%', '{:.2f}M'.format(ram_tot), ram_pct,
                                                        '{:.2f}M'.format(rom_tot), rom_pct, battery, name) + '\n'
            ...
            if not write_file and proc_result:
                ...
                total = row_formatter.format('0', str(cpu_user) + '%', '{:.2f}M'.format(mem_tot / 1024),
                                             mem_used / mem_tot, '{:.2f}M'.format(sto_tot / 1024), sto_used / sto_tot,
                                             battery, 'System') + '\n'
                ...
                sys.stdout.write('Press ctrl + c to stop sampling and save result to ' + outfile + ' ' + animation[
                    animation_idx % len(animation)] + '\r')
                animation_idx += 1
